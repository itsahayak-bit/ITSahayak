...
function handleGetStatus(params) {
  try {
    ...
    let todayRows = dataRows.filter(row => {
      ...
      const emailOk  = (iEmail !== undefined && empEmail && rowEmail)
                       ? rowEmail === empEmail : true;
      return rowDate === date && rowId === empId && emailOk;
    });

    // Step 2: Fallback — empId + date only (handles old/demo records)
    if (todayRows.length === 0) {
      todayRows = dataRows.filter(row => {
        return String(row[iDate]  || '').trim() === date &&
               String(row[iEmpId] || '').trim().toLowerCase() === empId;
      });
    }
